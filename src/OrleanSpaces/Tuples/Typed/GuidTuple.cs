    ...
    public bool Equals(GuidTuple other)
    {
        if (Vector128.IsHardwareAccelerated)
        {
            ...
            for (int i = 0; i < Length; i++)
            {
                ...
                ref Vector128<byte> vLeft = ref Extensions.Transform<Guid, Vector128<byte>>(ref fields[i]);
                ref Vector128<byte> vRight = ref Extensions.Transform<Guid, Vector128<byte>>(ref other.fields[i]);
