...
Console.WriteLine($"WORKER: Peek-ing a tuple that matches template {template} in a callback fashion...");
...
Console.WriteLine($"WORKER: Checking if {tuple} is still in space: {!(agent.Peek(template)).IsEmpty}");
